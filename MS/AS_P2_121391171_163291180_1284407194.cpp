...
void insertEmployee(MYSQL* conn, struct Employee emp)
{
	...
	else
	{
		...
		string query =
			"INSERT INTO employees (employeeNumber, lastName, firstName, email, extension, jobTitle, reportsTo ,officeCode) VALUES ('" + to_string(emp.employeeNumber) + "', '" + emp.lastName + "', '" + emp.firstName + "', '" + emp.email + "', '" + emp.extension + "', '" + emp.jobTitle + "', '" + emp.reportsTo  + "', '1');";
		const char* q = query.c_str();
		int exequery;
		exequery = mysql_query(conn, q);
		mysql_query(conn, q);
...
void deleteEmployee(MYSQL* conn, int employeeNumber)
{
	...
	{
		int exequery;
		string query1 = "update employees set reportsTo = 1002 where reportsTo =" + to_string(employeeNumber) + "; ";
		...
		string query2 = "update customers set salesRepEmployeeNumber = 1002 where salesRepEmployeeNumber =" + to_string(employeeNumber) + "; ";
		...
		string query3 = "DELETE FROM employees WHERE employeeNumber = " + to_string(employeeNumber) + ";";
		const char* q3 = query3.c_str();
		exequery = mysql_query(conn, q3);
		if (!exequery)
		{
